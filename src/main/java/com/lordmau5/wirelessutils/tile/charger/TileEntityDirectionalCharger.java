...
public class TileEntityDirectionalCharger extends TileEntityBaseCharger implements
    ...
    public int getEnergyCost(double distance, boolean isInterdimensional) {
        int cost = ModConfig.chargers.directionalCharger.maximumCost;
        if ( !isInterdimensional ) {
            int dimCost = 0;
            if ( distance > 0 )
                dimCost = (int) Math.floor(
                        (ModConfig.chargers.directionalCharger.costPerBlock * distance) +
                                (ModConfig.chargers.directionalCharger.costPerBlockSquared * (distance * distance))
                );
    ...
    public Object getGuiClient(InventoryPlayer inventory) {
        return new GuiDirectionalCharger(inventory, this);
    }

    @Override
    public Object getGuiServer(InventoryPlayer inventory) {
        return new ContainerDirectionalCharger(inventory, this);
    }
}
