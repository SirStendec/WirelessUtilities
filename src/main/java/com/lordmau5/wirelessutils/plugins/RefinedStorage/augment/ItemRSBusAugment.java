...
public class ItemRSBusAugment extends ItemAugment implements IUpdateableItem, IGuiItem, INBTPreservingIngredient {
    ...
    public void addInformation(@Nonnull ItemStack stack, @Nullable World worldIn, @Nonnull List<String> tooltip, ITooltipFlag flagIn) {
        ...
        final String name = "item." + WirelessUtils.MODID + ".bus_augment";
    ...
    @Nonnull
    public ItemStack setEnergyMode(@Nonnull ItemStack stack, @Nullable BusTransferMode mode) {
        if ( stack.isEmpty() || stack.getItem() != this )
        ...
        if ( mode == null || mode == BusTransferMode.BOTH )
    ...
    @Nonnull
    public BusTransferMode getEnergyMode(@Nonnull ItemStack stack) {
        if ( !stack.isEmpty() && stack.getItem() == this && stack.hasTagCompound() ) {
            NBTTagCompound tag = stack.getTagCompound();
            if ( tag != null && tag.hasKey("Energy", Constants.NBT.TAG_BYTE) )
                return BusTransferMode.byIndex(tag.getByte("Energy"));
        }

        return BusTransferMode.BOTH;
    }


    @Nonnull
    public ItemStack setFluidMode(@Nonnull ItemStack stack, @Nullable BusTransferMode mode) {
        if ( stack.isEmpty() || stack.getItem() != this )
            return ItemStack.EMPTY;
    ...
    @Nonnull
    public BusTransferMode getFluidMode(@Nonnull ItemStack stack) {
        if ( !stack.isEmpty() && stack.getItem() == this && stack.hasTagCompound() ) {
            NBTTagCompound tag = stack.getTagCompound();
            if ( tag != null && tag.hasKey("Fluid", Constants.NBT.TAG_BYTE) )
                return BusTransferMode.byIndex(tag.getByte("Fluid"));
        }
    ...
    @Nonnull
    public ItemStack setItemsMode(@Nonnull ItemStack stack, @Nullable BusTransferMode mode) {
        if ( stack.isEmpty() || stack.getItem() != this )
            return ItemStack.EMPTY;
    ...
    @Nonnull
    public BusTransferMode getItemsMode(@Nonnull ItemStack stack) {
        if ( !stack.isEmpty() && stack.getItem() == this && stack.hasTagCompound() ) {
            NBTTagCompound tag = stack.getTagCompound();
            if ( tag != null && tag.hasKey("Items", Constants.NBT.TAG_BYTE) )
                return BusTransferMode.byIndex(tag.getByte("Items"));
        }
    ...
    public boolean canApplyToDelegate(@Nonnull ItemStack stack, @Nonnull Class<? extends IAugmentable> klass) {
        return IBusAugmentable.class.isAssignableFrom(klass) && TileEntityBaseNetwork.class.isAssignableFrom(klass);
    }
